...
pclRegister <- function(meshRef, meshTar, radiusSearch=100, imageWidth=8, maxIt=500, numberOfSample=3, correspondenceRandomness=5, similarityThreshold=0.9, inlierFraction=0.25)
	{
		...
		tmp <- .Call("Rpclregister", vbRef, vbTar, normalsRef, normalsTar, radiusSearch, imageWidth, maxIt, numberOfSample, correspondenceRandomness, similarityThreshold, inlierFraction)
		tmp$it <- meshRef$it
		tmp$it <- meshRef$it
		return(tmp)
	}